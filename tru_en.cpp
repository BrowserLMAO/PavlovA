...
Tru_en::Tru_en(const std::__cxx11::string p_full_path): Ttranslator(p_full_path, "en_dict.txt", "ru_en_dict.txt") {
}

std::string Tru_en::translate(const std::__cxx11::string clause) {
  std::string result;
  ...
  try {
    ...
    if (exc.code == not_found_in_transl_words)
      throw_mess("Перевод слова " + exc.mess + " в русско-английском словаре не найден !");
    else if (exc.code == not_found_in_words)
      throw_mess("Определение слова " + exc.mess + " в английском морфологическом словаре не найдено !");
  }

  //  copy(transl_words_list.begin(), transl_words_list.end(), std::ostream_iterator<std::string>(std::cout, "\n")); //Вывод на экран
  return result;
}

void Tru_en::transl_by_words() {
...
void Tru_en::add_word_to_list(std::__cxx11::string word) {
