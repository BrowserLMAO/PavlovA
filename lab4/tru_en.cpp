...
Tru_en::Tru_en(const std::string p_full_path): Ttranslator(p_full_path, "en_dict.txt", "ru_en_dict.txt") {
  //Ввод русского алфавита
...
std::string Tru_en::translate(const std::string clause) {
  std::string result;
  ...
  try {
    ...
    else if (exc.code == invalid_symbol)
      throw_mess("Ошибочный символ " + exc.mess);
  }
...
void Tru_en::add_word_to_list(std::string word) {
