...
// Используется окрестность фон Неймана
void Tlife::get_coor_cell_N(byte x, byte y) {
  ...
  //  }
  // "Скручиваем" плоскость в тор
  coor_arr[0].x = x < 1 ? WIDTH - 1 : x - 1;
  coor_arr[0].y = y ;
  coor_arr[1].x = x == WIDTH - 1 ? 0 : x + 1;
  coor_arr[1].y = y ;
  coor_arr[2].x = x;
  coor_arr[2].y = y == HEIGHT - 1 ? 0 : y + 1;
  coor_arr[3].x = x;
  coor_arr[3].y = y < 1 ? HEIGHT - 1 : y - 1;
}

byte Tlife::get_analyze_res(byte x, byte y) {
  byte sum = 0;
  byte result = 0;
  // Получить координаты соседствующих с заданной клеток (окрестность фон Неймана)
  get_coor_cell_N(x, y);
